...
RSpec.describe PostToImagesJob, type: :job do
  ...
  let(:user){instance_double("User", :username => "dilpickle", :email => "test@testing.com")}
    before :each do
      ...
      @xml = File.read("#{Rails.root}/spec/fixtures/vra_no_display.xml")
      ...
      @new_record.save!
      stub_request(:post, "https://127.0.0.1:3333/multiresimages").with(:body => {"accession_nbr"=>"12367", "from_menu"=>"true", "path"=>"#{@path}", "xml" => @xml}, :headers => {'Accept'=>'*/*; q=0.5, application/xml', 'Accept-Encoding'=>'gzip, deflate', 'Content-Length'=>'4168', 'Content-Type'=>'application/x-www-form-urlencoded', 'User-Agent'=>'Ruby'}).to_return(:status => 200, :body => "<response><returnCode>Publish successful</returnCode><pid>inu:dil-321hdsjfhjs778</pid></response>", :headers => {})
    end
