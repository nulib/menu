...
describe "Adds display fields to VRA:image" do
  it "successfully" do
    vra_without_display = File.read( Rails.root.join( 'spec', 'fixtures', 'vra_no_display.xml' ))
    ...
    expect( vra ).to be_equivalent_to( vra_with_display ).respecting_element_order
  end
end
